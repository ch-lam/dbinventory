...
-- Ce script est le contenu de sql/oracle_lms.sql (script officiel Oracle LMS).
...
SELECT 'Multitenant'                                         , 'Oracle Multitenant'                                      , '^1[289]\.|^2[0-9]\.'                          , 'C003'    from dual union all
SELECT 'Multitenant'                                         , 'Oracle Pluggable Databases'                              , '^1[289]\.|^2[0-9]\.'                          , 'C003'    from dual union all
    ...
    case  when CONDITION = 'BUG'
          ...
          when     detected_usages > 0
               ...
               and (    trim(CONDITION) is null
                     or CONDITION_MET     = 'TRUE'
                    and CONDITION_COUNTER = 'FALSE' )
...
select m.PRODUCT, m.CONDITION, m.MVERSION,
       case
             when CONDITION = 'C001' and (   regexp_like(to_char(FEATURE_INFO), 'compression[ -]used:[ 0-9]*[1-9][ 0-9]*time', 'i')
                                         and FEATURE_INFO not like '%(BASIC algorithm used: 0 times, LOW algorithm used: 0 times, MEDIUM algorithm used: 0 times, HIGH algorithm used: 0 times)%'
  ...
  where nvl(CONDITION, '-') != 'INVALID'
    and not (CONDITION = 'C003' and CON_ID not in (0, 1))
)
select
    grouping_id(pfs.CON_ID)||','||
    pfs.CON_ID||','||
    decode(grouping_id(pfs.CON_ID), 1, '--ALL--', max(pfs.CON_NAME))||','||
    pfs.PRODUCT||','||
    decode(max(USAGE),
          ...
          'UNKNOWN')||','||
    max(pfs.LAST_SAMPLE_DATE)||','||
    min(pfs.FIRST_USAGE_DATE)||','||
    max(pfs.LAST_USAGE_DATE)||','||
	dbsn.NAME||','||
	inst.HOST_NAME
  from PFUS pfs, v$database dbsn, v$instance inst
  where pfs.USAGE in ('2.NO_CURRENT_USAGE', '4.PAST_USAGE', '5.PAST_OR_CURRENT_USAGE', '6.CURRENT_USAGE')
  group by rollup(pfs.CON_ID), pfs.PRODUCT,dbsn.NAME,inst.HOST_NAME
  having not (max(pfs.CON_ID) in (-1, 0) and grouping_id(pfs.CON_ID) = 1)
order by 1 desc, pfs.CON_ID, decode(substr(pfs.PRODUCT, 1, 1), '.', 2, 1), pfs.PRODUCT;
