#!/bin/bash
# =============================================================================
# get_info_oracle_security — collecte sécurité Oracle
# =============================================================================
set -euo pipefail
...
init_temps sec_parameter sec_network_enc sec_profile sec_default_profile \
           sec_admin_high_privs sec_high_role_privs sec_high_sys_privs    \
           sec_high_tab_privs sec_default_user_pwd sec_usermig            \
           sec_tablespace sec_audit
...
while IFS=';' read -r db_env lic db; do
    ...
    IS_CDB=$([[ "${is_cdb}" == "YES" ]] && echo "YES" || echo "NO")
    log "  rôle=${db_role} cdb=${IS_CDB} ver=${db_ver}"

    # Standby : aucune collecte sécurité
    [[ "${db_role}" == *STANDBY* ]] && continue

    # ── Paramètres et chiffrement réseau (tous) ───────────────────────────────
    sql_run "${db}" <<SQL >> /dev/null
SPOOL ${LIST_DIR}/sec_parameter_temp.csv APPEND
SELECT '"'||dbs.NAME||'","'||REPLACE(prm.VALUE,'"','')||'","'||prm.name||'"'
FROM v\$parameter prm, v\$database dbs
...
UNION ALL
SELECT '"'||dbs.NAME||'","'||REPLACE(prm.VALUE,'"','')||'","'||prm.PARAMETER||'"'
FROM v\$option prm, v\$database dbs
...
SPOOL OFF
SPOOL ${LIST_DIR}/sec_network_enc_temp.csv APPEND
...
SQL
    ...
    if [[ "${IS_CDB}" == "YES" ]]; then
        sql_run "${db}" <<SQL >> /dev/null
SPOOL ${LIST_DIR}/sec_profile_temp.csv APPEND
...
SPOOL OFF
SPOOL ${LIST_DIR}/sec_default_profile_temp.csv APPEND
...
SPOOL OFF
SPOOL ${LIST_DIR}/sec_usermig_temp.csv APPEND
-- SYS.USER\$MIG : table residuelle de migration contenant des hash de mots de passe
-- au format legacy 10G (DES). Sa presence indique que des comptes ont encore un hash
-- faible actif en parallele du hash moderne, exploitable par brute-force.
...
SQL
    ...
    # ── Privilèges élevés ─────────────────────────────────────────────────────
    sql_run "${db}" <<SQL >> /dev/null
SPOOL ${LIST_DIR}/sec_admin_high_privs_temp.csv APPEND
...
SQL
    ...
    if [[ "${IS_CDB}" == "YES" ]]; then
        sql_run "${db}" <<SQL >> /dev/null
SPOOL ${LIST_DIR}/sec_high_role_privs_temp.csv APPEND
...
SPOOL OFF
SPOOL ${LIST_DIR}/sec_high_sys_privs_temp.csv APPEND
...
SPOOL OFF
SPOOL ${LIST_DIR}/sec_high_tab_privs_temp.csv APPEND
...
SPOOL OFF
SPOOL ${LIST_DIR}/sec_default_user_pwd_temp.csv APPEND
...
SQL
    ...
    # ── Tablespaces chiffrés (primaire uniquement) ────────────────────────────
    if [[ "${IS_CDB}" == "YES" ]]; then
        sql_run "${db}" <<SQL >> /dev/null
SPOOL ${LIST_DIR}/sec_tablespace_temp.csv APPEND
...
SQL
    ...
    # ── Audit (12c+ : politique unifiée / 11g : audit classique) ─────────────
    if [[ "${db_ver}" -ge 12 ]]; then
        if [[ "${IS_CDB}" == "YES" ]]; then
            sql_run "${db}" <<SQL >> /dev/null
SPOOL ${LIST_DIR}/sec_audit_temp.csv APPEND
...
SQL
            ...
            sql_run "${db}" <<SQL >> /dev/null
SPOOL ${LIST_DIR}/sec_audit_temp.csv APPEND
SELECT '"'||(SELECT name FROM v\$database)||'","NON CONTAINER","ORA_CIS_RECOMMENDATIONS","'||
...
# ── Finalisation ──────────────────────────────────────────────────────────────
log "Finalisation..."
finalize_lst "${LIST_DIR}/sec_parameter_temp.csv"        "${LIST_DIR}/oracle_security_parameter.csv"
finalize_lst "${LIST_DIR}/sec_network_enc_temp.csv"      "${LIST_DIR}/oracle_security_network_encryption.csv"
finalize_lst "${LIST_DIR}/sec_profile_temp.csv"          "${LIST_DIR}/oracle_security_profile.csv"
finalize_lst "${LIST_DIR}/sec_default_profile_temp.csv"  "${LIST_DIR}/oracle_security_default_profile.csv"
finalize_lst "${LIST_DIR}/sec_usermig_temp.csv"          "${LIST_DIR}/oracle_security_usermig_table_exist.csv"
finalize_lst "${LIST_DIR}/sec_admin_high_privs_temp.csv" "${LIST_DIR}/oracle_security_admin_high_privs.csv"
finalize_lst "${LIST_DIR}/sec_high_role_privs_temp.csv"  "${LIST_DIR}/oracle_security_high_role_privs.csv"
finalize_lst "${LIST_DIR}/sec_high_sys_privs_temp.csv"   "${LIST_DIR}/oracle_security_high_sys_privs.csv"
finalize_lst "${LIST_DIR}/sec_high_tab_privs_temp.csv"   "${LIST_DIR}/oracle_security_high_tab_privs.csv"
finalize_lst "${LIST_DIR}/sec_default_user_pwd_temp.csv" "${LIST_DIR}/oracle_security_default_user_pwd.csv"
finalize_lst "${LIST_DIR}/sec_tablespace_temp.csv"       "${LIST_DIR}/oracle_security_tablespace.csv"
finalize_lst "${LIST_DIR}/sec_audit_temp.csv"            "${LIST_DIR}/oracle_security_audit_enable.csv"

report_errors "Oracle inventory (sécurité): erreurs de connexion"
log "Terminé."
