...
-- Oracle version: FORMAT(x,0) -> TO_CHAR(ROUND(x))

-- Oracle Licensing Core Usage
SELECT license_type, os_type, SUM(num_cpus) AS vcpu,
       CASE WHEN os_type='Linux x86 64-bit' THEN TO_CHAR(ROUND(SUM(num_cpus)/2))   ELSE TO_CHAR(ROUND(SUM(num_cpus)/8)) END AS num_core,
       CASE WHEN os_type='Linux x86 64-bit' THEN TO_CHAR(ROUND((SUM(num_cpus)/2)/2)) ELSE TO_CHAR(ROUND(SUM(num_cpus)/8)) END AS core_with_core_factor
FROM oracle_hosts
