...
--   CONCAT() -> || ; TO_CHAR() on NUMBER columns (config_value, is_auto_close_on, is_trustworthy_on)
...
WHERE servername || name || TO_CHAR(config_value) NOT IN (
    SELECT servername || name || TO_CHAR(config_value)
...
WHERE servername || db_name || TO_CHAR(is_auto_close_on) || TO_CHAR(is_trustworthy_on) NOT IN (
    SELECT servername || db_name || TO_CHAR(is_auto_close_on) || TO_CHAR(is_trustworthy_on)
...
WHERE servername || db_name || key_type || key_name NOT IN (
    SELECT servername || db_name || key_type || key_name
...
WHERE db_name || permission_name || state_desc NOT IN (
    SELECT db_name || permission_name || state_desc
...
WHERE servername || login_name NOT IN (
    SELECT servername || login_name
...
WHERE servername || db_name || user_name NOT IN (
...
WHERE db_name || username || permission_name || state_desc NOT IN (
    SELECT db_name || username || permission_name || state_desc
...
WHERE servername || auditactionname NOT IN (
    SELECT servername || auditactionname
